...
//Declare Vars
int batt_charge, batt_health, rated_voltage, rated_current1, rated_current2, batt_voltage, batt_temp, bms1, bms2;
...
void build() {
  ...
  GP.UPDATE("charge_allowed_led,discharge_allowed_led,gofull_led,batt_voltage_state,batt_soc_state,batt_soh_state,batt_temp_state,batt_curr_state,batt_limmits_state,esponline_time");
...
//Home Tab
  ...
  M_BLOCK(GP_TAB, "", "Поточне Навантаження", GP.TITLE(current_str, "batt_curr_state"););
  M_BLOCK(GP_TAB, "", "Ліміти батареї", GP.TITLE(limmits_str, "batt_limmits_state"););
  GP.BREAK();
  GP.HR();
  M_BLOCK(GP_THIN, "", "Час Онлайн Контролера",GP.LABEL(esponline_str, "esponline_time"););
  GP.NAV_BLOCK_END();
  GP.NAV_BLOCK_BEGIN();
  // Inverter Tab
  ...
  GP.BREAK();
  GP.HR();
  GP.PLAIN("Ці параметри можуть інгноруватися вашим пристроєм, все залнежить від моделі, прошивки та виробника", "disclaimer", GP_YELLOW);
  GP.HR();
  GP.BREAK();
  ...
  GP.FORM_BEGIN("/wifi");
  GP.SWITCH("wifi_mode", data.wifi_mode); GP.LABEL("Під'єднуватись до wifi");GP.BREAK();
...
void web_action() {
  if (portal.form()) {
    ...
    if (portal.form("/wifi")) {
      ...
      if (data.wifi_mode){Serial.print("Station Mode ");} else {Serial.print("Access Point Mode ");} Serial.print("ssid: "); Serial.print(data.client_ssid); Serial.print(" password: "); Serial.println(data.client_pswd);
      memory.update();
      needsReset = true;
    }
  }
  if (portal.update()) {
    if (DEBUG){Serial.print("Got Update Signal ");Serial.println(portal.updateName());}
    if (portal.update("rst_info")){
      portal.answer("Налаштування було скинуто!");
      needsInit = true;
    }
    if (portal.update("charge_allowed_led")) portal.answer(charge_allowed);
    if (portal.update("discharge_allowed_led")) portal.answer(discharge_allowed);
    if (portal.update("gofull_led")) portal.answer(gofull);
    if (portal.update("batt_voltage_state")) portal.answer(voltage);
    if (portal.update("batt_soc_state")) portal.answer(charge_str);
    if (portal.update("batt_soh_state")) portal.answer(health_str);
    if (portal.update("batt_temp_state")) portal.answer(temp_str);
    if (portal.update("batt_curr_state")) portal.answer(current_str);
    if (portal.update("batt_limmits_state")) portal.answer(limmits_str);
    if (portal.update("esponline_time")) portal.answer(esponline_str);
  ...
  if (portal.click()) {
    ...
    if (portal.click("target_charge")){data.target_chrg = portal.getInt();memory.update();}
    if (portal.click("target_discharge")){data.target_dischrg = portal.getInt();memory.update();}
    if (portal.click("full_charge_time")){data.full_charge_time = portal.getInt();}
  }
}
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (data.full_charge_time){gofull=true;} else {gofull=false;};

    health_str = String(batt_health) + "%";
    
    charge_str = String(batt_charge) + "%";

    drated_voltage = rated_voltage; drated_voltage = drated_voltage / 10;
    dbatt_voltage = batt_voltage; dbatt_voltage = dbatt_voltage / 100;
    voltage = String(dbatt_voltage) + " - " + String(drated_voltage); 

    dbatt_temp = batt_temp; dbatt_temp = dbatt_temp / 10 ; 
    temp_str = String(dbatt_temp) + "°C";

    dbatt_current = batt_current; dbatt_current = dbatt_current / 10;
    current_str = String(dbatt_current) + "A";
  
    drated_current1 = rated_current1; drated_current1 = drated_current1 / 10; 
    drated_current2 = rated_current2; drated_current2 = drated_current2 / 10;
    limmits_str = String(drated_current1) + "A / " + String(drated_current2) + "A";
...
void mcp_read(){
  if(!digitalRead(CAN0_INT))                         // If CAN0_INT pin is low, read receive buffer
  {
    ...
    ProcessData (rxId, rxBuf[0], rxBuf[1], rxBuf[2], rxBuf[3], rxBuf[4], rxBuf[5], rxBuf[6], rxBuf[7]);       
  }
}

byte f35e[8] = {'P', 'Y', 'L', 'O', 'N', ' ', ' ', ' '};         //deya protocol id
byte f370[8] = {'P', 'Y', 'L', 'O', 'N', 'D', 'I', 'Y'};         //sunny island optional id
...
void mcp_send(){ 
  ...
  if(DEBUG){Serial.print("Sending Frame 35C: ");Serial.print(f35c[1], BIN); Serial.print(" HEX: "); Serial.print(f35c[1], HEX); Serial.print(" ");}
...
void ProcessData (uint32_t cid, int d0, int d1, int d2, int d3, int d4, int d5, int d6, int d7) {
  // Try to parse Data
    switch(cid) {
      ...
      default:
      if (DEBUG){
       Serial.print(F("Unknown Frame with Adress:")); Serial.print(cid, HEX); Serial.print(" "); Serial.print("\t");
        Serial.print(d0, HEX);Serial.print(" ");Serial.print(d1, HEX);Serial.print(" ");
        Serial.print(d2, HEX);Serial.print(" ");Serial.print(d3, HEX);Serial.print(" ");
        Serial.print(d4, HEX);Serial.print(" ");Serial.print(d5, HEX);Serial.print(" ");
        Serial.print(d6, HEX);Serial.print(" ");Serial.print(d7, HEX);Serial.print(" ");
        Serial.println();
      }
        break;
    }
...
void PrintData (){
        Serial.print("Load:");Serial.print(dbatt_current);Serial.print("amps\t");
        Serial.print("RC:");Serial.print(drated_current1);Serial.print("/");Serial.print(drated_current2);Serial.print("\t");
        Serial.print("SOC:");Serial.print(batt_charge); Serial.print("%");Serial.print(" ");
        Serial.print("SOH:");Serial.print(batt_health); Serial.print("%");Serial.print(" ");
        Serial.print("T:");Serial.print(dbatt_temp); Serial.print("\t");
        Serial.print("V:");Serial.print(dbatt_voltage); Serial.print(" of "); Serial.print(drated_voltage);
        Serial.println();
}
...
void checkmac(){
  ...
  String licmacstr(licmac);String curmacstr(WiFi.macAddress());if (curmacstr != licmacstr) {while (1) {Serial.println("ESP Warmup!");delay(1000);};}
}
...
void setWifiSta(){
    ...
    while (WiFi.status() != WL_CONNECTED) {
      delay(1000);
      Serial.print(".");
      if (wifi_connect_wait_time < 20) { wifi_connect_wait_time++; } else{ break; }
    }
}
void setWifiAp(){
...
void initWifi(){
  if (data.wifi_mode){
    setWifiSta(); 
      if(WiFi.status() != WL_CONNECTED) {setWifiAp();}
    } else { setWifiAp(); }
